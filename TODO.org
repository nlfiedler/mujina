...
- [TODO] Write a design document
  - Why there is a redux store for each render process
...
- [DONE] See how https://github.com/erikras/redux-form can help
- [DONE] See how https://github.com/davidkpiano/react-redux-form can help
...
- [DONE] Decide what the routing will look like
- [DONE] Use https://reacttraining.com/react-router/ for routing
- [DONE] Read https://reacttraining.com/react-router/web/guides/redux-integration
- [DONE] Read https://github.com/reacttraining/react-router/tree/master/packages/react-router-redux
- [DONE] Look at the examples https://reacttraining.com/react-router/web/example/basic
...
- [DONE] Fire a redux action to signal received files
- [DONE] Remove electron-redux because it caused actions to be out of order
- [DONE] Show uploaded assets in a list with editable fields
  - [DONE] Sync problem with DROP_FILES action and route change
  - [DONE] Probably use a form of some sort for the asset details
  - [DONE] Figure out how react-redux-form can handle an array of things
  - [DONE] Fire actions when form values are updated to keep state updated
- [DONE] Commit edited data to the corresponding database records
  - [DONE] Use a saga to checksum dropped file paths to produce unique keys
  - [DONE] Somehow fire the route change to /upload
  - [DONE] NewFile[List] uses digest as the key instead of file path
  - [DONE] When dropped files are submitted, fire an action to upload files
  - [DONE] After the files are uploaded, need to update the tags, locations for each asset
  - [DONE] If there are errors, route to an error page (untested)
  - [DONE] If successful, route to home page
...
- [TODO] Show small thumbnails next to the new files (using "Media" component?)
- [TODO] Include form field for caption and add to api.uploadFiles()
- [TODO] Maybe a "save" button next to each dropped file, and another (at the top) to save everything at once
- [TODO] Design shortcuts for entering identical values for multiple dropped files
