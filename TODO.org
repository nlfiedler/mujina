...
**** TODO Replace ThumbnailRow with ThumbnailColumn that is just like filters
...
*** Thumbnail view
**** DONE Improve the thumbnail rendering (reduce jitter) [5/5]
- [X] Remove the onLoad handler, no longer needed
- [X] Use a placeholder image file of 300x300 pixels
- [X] Stretch the image to fit the size of the thumbnail from the search results
- [X] Get a prettier placeholder image because it is briefly visible
- [X] See if there is a way to smooth the transition from placeholder image
  - It flashes briefly when switching over to the real image

**** DONE The =ThumbnailRow= is truncating the results when there are many
***** DONE Limit ThumbnailRow =Container= to not grow to fit children
***** DONE Remove padding from =AssetPage= tiles
**** TODO Seems like AttributeList =div= is not an ideal solution
- It has a hard-coded width which surely is altering the layout
...
- Need to set the image dimensions to squeeze them into rows, filling all
  available space, and leaving only a small margin.
  - No need to resize the thumbnail, just set the =img= dimensions
- If the row has space, get the next image, scale down so they fit the row.
  - That is, select one asset at a time, until the combined width exceeds the container,
    then shrink the images to fit the width (including margins).
- The placeholder image should be 300x300 on disk and stretched to fit actual thumb size
- Need to adjust layout when window is resized

*** Quick Ones
**** TODO Use =.npmrc= to avoid creating =package-lock.json= file
*** Glitches
...
**** TODO Make the asset sidebar sticky so it does not scroll away
**** TODO Make the asset selector (thumbnail row) sticky so it does not scroll away
*** Bulk Edit
