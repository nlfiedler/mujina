...
- [TODO] Write a configuration page
  - Access via Preferences menu item
  - Set the host and port of the server
- [TODO] Get tanuki installed on FreeBSD (alongside old deployment)
- [TODO] Write a one-off migration tool
  - Assets continue to stay where they are
  - Read old documents from CouchDB (via http)
  - Write new documents to PouchDB

## Wifey Enablement

### Course of Action

- [TODO] Need remote host/port configuration
  - For now, hand-edit the electron-store config in ~/Library/Application Support/mujina/userData
...
- [TODO] Design a "folders" feature
  - Add a "folders" field to the database records
  - Store them as a list of strings
  - Each asset can be in multiple folders
  - Use multiple selection to quickly assign folders to assets
  - Show folders with the other filters, selectors
  - Wifey suggests "project" instead of folders, but folders is more general
...
- [TODO] Design shortcuts for entering identical values for multiple dropped files
  - Maybe select several and type into one field, auto copied to the others, keystroke by keystroke
